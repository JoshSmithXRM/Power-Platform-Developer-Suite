...
export class ImportJobViewerPanel extends BasePanel<ImportJobInstanceState, ImportJobPreferences> {
     ...
     * Used for handling context menu clicks on import job table rows
     */
    protected async handleOtherComponentEvent(componentId: string, eventType: string, data?: unknown): Promise<void> {
        ...
        if (componentId === 'importJobs-table' && eventType === 'contextMenuItemClicked') {
            ...
            switch (itemId) {
                case 'viewXml':
                    if (rowData?.id) {
                        // Pass undefined for environmentId - handler will use current environment from selector
     ...
     */
    protected async applyPreferences(prefs: ImportJobPreferences | null): Promise<void> {
        // No preferences to restore for this panel yet
        // Future: Could restore sort preferences, filter preferences, auto-refresh settings, etc.
